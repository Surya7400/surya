const firebaseConfig = {
    apiKey: "AIzaSyARqHa2HAItGNrfmuY5wug1SpB64Rom0",
...
function loadTasks() {
    tasksList.innerHTML = '';
    db.collection('todos').where('userId', '==', currentUser.uid).get().then(querySnapshot => {
        querySnapshot.forEach(doc => {
            const task = doc.data().task;
            const taskId = doc.id;
            const taskItem = document.createElement('li');
            taskItem.innerHTML = `<span>${task}</span> <button class="delete-task" data-id="${taskId}">Delete</button>`;
...
tasksList.addEventListener('click', event => { 
    if (event.target.classList.contains('delete-task')) {
        ...
        db.collection('todos').doc(taskId).delete().then(() => {
            loadTasks();
        }).catch(error => {
            console.error(error.message); 
